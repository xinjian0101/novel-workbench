...
def test_cli_create_show_stats_search_backup_and_export(tmp_path: Path, capsys) -> None:
    ...
    assert "# Renamed Novel Share Kit Index" in (tmp_path / "share-kit" / "renamed-novel-share-index.md").read_text(encoding="utf-8")
...
def test_demo_script_runs(capsys) -> None:
    ...
    assert "moon-archive-share-index.md" in captured.out
...
def test_pages_demo_script_builds_static_site(tmp_path: Path, capsys) -> None:
    ...
    assert '<link rel="manifest" href="https://xinjian0101.github.io/novel-workbench/site.webmanifest">' in index
    ...
    assert "https://xinjian0101.github.io/novel-workbench/site.webmanifest" in (output_dir / "sitemap.xml").read_text(encoding="utf-8")
    ...
    assert json.loads((output_dir / "site.webmanifest").read_text(encoding="utf-8"))["start_url"] == "https://xinjian0101.github.io/novel-workbench/index.html"
...
def test_public_link_verifier_lists_launch_targets_offline(capsys) -> None:
    ...
    assert "CHECK Pages web manifest: https://xinjian0101.github.io/novel-workbench/site.webmanifest" in captured.out
