...
The generated scripts complete top-level commands such as `create`, `dashboard`, `export-dashboard`, `focus`, `handoff`, `momentum`, `board`, `outline`, `plan`, `review`, `revision`, `migrate`, `add-character`, `add-location`, `add-scene`, `add-progress`, `update-progress`, `delete-progress`, `set-deadline`, `stats`, `export`, `export-pack`, `backup`, and `restore-backup`. They do not yet complete project slugs, file paths, or option values.
