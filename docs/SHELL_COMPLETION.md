...
The generated scripts complete top-level commands such as `create`, `outline`, `add-scene`, `add-progress`, `set-deadline`, `stats`, `export`, `backup`, and `restore-backup`. They do not yet complete project slugs, file paths, or option values.
