...
The generated scripts complete top-level commands such as `create`, `dashboard`, `outline`, `plan`, `migrate`, `add-character`, `add-location`, `add-scene`, `add-progress`, `set-deadline`, `stats`, `export`, `backup`, and `restore-backup`. They do not yet complete project slugs, file paths, or option values.
